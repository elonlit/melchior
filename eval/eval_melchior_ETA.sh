...
run_basecalling() {
    ...
    python -m eval.basecall data/test/${dataset} -m melchior -b 128 | 
...
for dataset in "${datasets[@]}"; do
    output_file="eval/melchior_outputs/${dataset}/output.fasta"
...
for dataset in "${datasets[@]}"; do
    ...
    input="eval/melchior_outputs/${dataset}/output.fasta"
    output="eval/melchior_outputs/${dataset}/aligned.sam"
    if [ -f "$output" ]; then
...
for dataset in "${datasets[@]}"; do
    echo "For ${dataset}:"
    python -m eval.accuracy eval/melchior_outputs/${dataset}/aligned.sam data/test/transcriptomes/${dataset%%-*}_reference.fasta > eval/melchior_outputs/${dataset}/accuracy.txt
done
