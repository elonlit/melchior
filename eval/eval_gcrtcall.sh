...
echo "Benchmarking GCRTcall..."

mkdir -p eval/gcrtcall_outputs
cd eval/gcrtcall_outputs
...
for dataset in "${datasets[@]}"; do
    output_file="eval/gcrtcall_outputs/${dataset}/output.fasta"
...
for dataset in "${datasets[@]}"; do
    ...
    input="eval/gcrtcall_outputs/${dataset}/output.fasta"
    output="eval/gcrtcall_outputs/${dataset}/aligned.sam"
    if [ -f "$output" ]; then
...
for dataset in "${datasets[@]}"; do
    echo "For ${dataset}:"
    python -m eval.accuracy eval/gcrtcall_outputs/${dataset}/aligned.sam data/test/transcriptomes/${dataset%%-*}_reference.fasta > eval/gcrtcall_outputs/${dataset}/accuracy.txt
done
