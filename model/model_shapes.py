...
from utils.utils import MelchiorDataset
from torch.utils.data import DataLoader
import matplotlib.pyplot as plt

data = MelchiorDataset(path="data/rna-train.hdf5")
data_loader = DataLoader(dataset=data, batch_size=32, shuffle=True, pin_memory=True)

# Display image and label.
for batch, data in enumerate(data_loader):
    if len(data) == 4:
        event, event_len, label, label_len = data
    else:  # len(data) == 5
        event, event_len, label, label_len, idx = data
    
    print(f"Batch: {batch}")
    print(f"Event shape: {event.shape}, Event length: {event_len.shape}")
    print(f"Label shape: {label.shape}, Label length: {label_len.shape}")
    print(event[1].shape, label[1].shape, event_len[7], label_len[2])

    print(label[8])
    if len(data) == 5:
        print(f"Index: {idx.shape}")
