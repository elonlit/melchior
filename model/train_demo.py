...
class network(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.final(x)
        print("After final linear layer:", x.shape)
...
def train(config = None, args = None, arch = None):
    ...
    for epoch in range(config.epochs):
        ...
        for i, (event, event_len, label, label_len) in enumerate(data_loader):
            ...
            print(event.shape, label.shape, event_len.shape, label_len.shape)
...
if __name__ == "__main__":
    ...
    # arch = eval(open("model/rnaarch", "r").read())
    # train(config=defaultconfig, args=rna_default, arch=arch)
    config = objectview(defaultconfig)
    model = network(config=config, arch=rna_default, seqlen=4096)

    # Create a sample input tensor
    # Assuming input is a 1D signal with 4096 time steps
    # sample_input = torch.randn(128, 1, 4096)  # (batch_size, channels, sequence_length)
    ...
    # train(config=defaultconfig, args=None, arch=rna_default)
